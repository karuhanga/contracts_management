    ...
    app_state = {
        ...
        methods: {
            ...
            confirm_delete: function () {
                ...
                fetch(CORE_URL + entity + separator + this.in_delete + "/delete").then(function (value) {
                    ...
                    if (value.ok) {
                        ...
                        toast_err("Failed to delete " + cs + " (You need to delete all contracts first)");
                    }
            ...
            clear_all: function () {
                ...
                for (contract in contracts_sec) {
                    contracts_sec[contract]['popup_active'] = false;
                }
            },
            ...
            get_color: function (contract) {
                let expiry = contract.expiry_date.split("-");
                let expiry_date = new Date(expiry[0], expiry[1], expiry[2]);
                let today = new Date();
                if (expiry_date < today) {
                    return "is-danger";
                }
                expiry_date.setDate(expiry_date.getDate() - DANGER_TIME)
                if (expiry_date < today) {
                    return "is-warning";
                }
